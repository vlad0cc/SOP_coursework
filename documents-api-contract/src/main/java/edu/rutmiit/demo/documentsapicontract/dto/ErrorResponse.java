...
public record ErrorResponse(
        ...
        @Schema(description = "URI запроса, приведшего к ошибке", example = "/api/documents/42")
        String instance,
    ...
    public record FieldError(
            ...
            @Schema(description = "Имя невалидного поля", example = "documentNumber")
            String field,

            @Schema(description = "Значение, которое было отклонено", example = "123")
            Object rejectedValue,

            @Schema(description = "Причина отклонения", example = "Некорректный номер документа. Допустимый формат: X-XXX-XXX")
            String message
    ) {}
}
