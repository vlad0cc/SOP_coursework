...
import edu.rutmiit.demo.grpcanalytics.service.DocumentAnalyticsServiceImpl;
    ...
    public void start() {
        try {
            server = ServerBuilder.forPort(grpcPort)
                    .addService(new DocumentAnalyticsServiceImpl())
                    .build()
                    .start();

            running = true;
            log.info("gRPC-сервер запущен на порту {}", grpcPort);
            log.info("Сервис: DocumentAnalytics.AnalyzeDocument()");
